...
class Counters
{
    ...
    public function forumNew($mod = 0)
    {
        if ($this->systemUser->isValid()) {
            ...
            if ($mod) {
                return '<a href="' . $this->homeurl . '/forum/index.php?act=new&amp;do=period">' . _t('Show for Period', 'system') . '</a>' .
                    ($total ? '<br><a href="' . $this->homeurl . '/forum/index.php?act=new">' . _t('Unread', 'system') . '</a>&#160;<span class="red">(<b>' . $total . '</b>)</span>' : '');
            } else {
                return $total;
            }
        } else {
            if ($mod) {
                return '<a href="' . $this->homeurl . '/forum/index.php?act=new">' . _t('Last activity', 'system') . '</a>';
            } else {
                return false;
            }
        }
