...
class Comments
{
        ...
        switch ($mod) {
            case 'reply':
                ...
                if ($this->systemUser->isValid() && $this->item && $this->access_reply && !$this->ban) {
                    ...
                    $req = $this->db->query("SELECT * FROM `" . $this->comments_table . "` WHERE `id` = '" . $this->item . "' AND `from_id` = '" . $this->sub_id . "' LIMIT 1");
                    ...
                    if (isset($_GET['yes'])) {
                        ...
                        if ($req->rowCount()) {
                            ...
                            if (isset($_GET['all'])) {
                                ...
                                $count = $this->db->query("SELECT COUNT(*) FROM `" . $this->comments_table . "` WHERE `from_id` = '" . $this->sub_id . "' AND `user_id` = '" . $res['user_id'] . "'")->fetchColumn();
                                $this->db->exec("DELETE FROM `" . $this->comments_table . "` WHERE `from_id` = '" . $this->sub_id . "' AND `user_id` = '" . $res['user_id'] . "'");
                            } else {
            ...
            default:
                ...
                if ($this->total) {
                    ...
                    WHERE `from_id` = '" . $this->sub_id . "' ORDER BY `subid` DESC LIMIT $start, $kmess");
                    $i = 0;
    ...
    {
        $total = $this->db->query("SELECT COUNT(*) FROM `" . $this->comments_table . "` WHERE `from_id` = '" . $this->sub_id . "'")->fetchColumn();
