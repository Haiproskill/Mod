...
$req = $db->query("SELECT `id`, `name`, `sex`, `status`, `rights`, `balansAdd` FROM `users` WHERE `preg` = 1 ORDER BY `balansAdd` DESC LIMIT $start, $kmess");
if ($req->rowCount()) {
	$i = 0;
	while ($res = $req->fetch()) {
        ...
        $top[$i]['balans'] = $tools->balans($res['balansAdd']);
