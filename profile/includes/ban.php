...
switch ($mod) {
    ...
    default:
        // История нарушений
        echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4><a href="?user=' . $user['id'] . '"><b>' . _t('Profile') . '</b></a>&#160;|&#160;' . _t('Violations History') . '</h4></div>';
        // Меню
        $menu = [];
        ...
        if ($total > $kmess) {
            echo '<div class="topmenu">' . $tools->displayPagination('?act=ban&amp;user=' . $user['id'] . '&amp;', $start, $total, $kmess) . '</div>';
        }
        echo '</div>';
}
