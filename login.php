...
if ($systemUser->isValid()) {
    ...
    echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . _t('Login', 'system') . '</h4></div>';
    $error = [];
    $captcha = false;
    $display_form = 1;
            ...
            if ($systemUser['failed_login'] > 2) {
                if ($user_code) {
                    ...
                    echo '<form action="login.php' . ($id ? '?id=' . $id : '') . '" method="post">' .
                        ...
                        '<table cellpadding="0" cellspacing="0"><tr><td style="padding-right: 5px;">' .
                        '<img src="captcha.php?r=' . rand(1000, 9999) . '" alt="' . _t('Verification code', 'system') . '" border="1" />' .
                        '</td><td>' .
                        '<div class="form-group">' .
                        '<input type="text" maxlength="5"  name="code" required="required" />' .
                        '<label class="control-label" for="input">' . _t('Enter verification code', 'system') . '</label><i class="bar"></i>' .
                        '</div>' .
                        '</td></tr></table>' .
                        ...
                        '<button type="submit" name="submit" class="button"><span>' . _t('Continue', 'system') . '</span></button>' .
                        '</div></center></div></form>';
                }
            }
    ...
    if ($display_form) {
        ...
        echo $info;
        echo '<div class="list1"><form action="login.php" method="post">' .
            '<div class="form-group">' .
            '<input id="nick" type="text" name="n" value="' . htmlentities($user_login, ENT_QUOTES, 'UTF-8') . '" maxlength="20" required="required" />' .
            '<label class="control-label" for="nick">' . _t('Username', 'system') . '</label><i class="bar"></i>' .
            '</div>' .
            '<div class="form-group">' .
            '<input id="pass" type="password" name="p" maxlength="20" required="required" />' .
            '<label class="control-label" for="pass">' . _t('Password', 'system') . '</label><i class="bar"></i>' .
            '</div>' .
            '<div class="checkbox">' .
            '<label><input type="checkbox" name="mem" value="1" checked="checked"/><i class="helper"></i>' . _t('Remember', 'system') . '</label>' .
            '</div>' .
            '<div class="button-container"><button  type="submit" name="submit" class="button"><span>' . _t('Login', 'system') . '</span></button></div>' .
            '</form></div>' .
            '<div class="list1"><a href="registration/">' . _t('Registration', 'system') . '</a></div>' .
            '<div class="list1"><a href="profile/skl.php?continue">' . _t('Forgot password?', 'system') . '</a></div>';
    }
    echo '</div>';
}
