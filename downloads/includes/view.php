    ...
    case 'mp3':
       ...
       /// $getID3->encoding = 'cp1251';
        ...
        if ($mp3info) {
            ...
            if (isset($tagsArray['unsynchronised_lyric'][0])) {
                $text_info .= '<b>' . _t('Lời') . '</b>: ' . $tools->checkout($tagsArray['unsynchronised_lyric'][0], 1) . '<br>';
            }
            if (isset($getid['comments']['picture']['0']['data'])) {
                $picture = $getid['comments']['picture']['0']['data'];
                $picture_mime = $getid['comments']['picture']['0']['image_mime'];
                $text_info .= '<b>' . _t('img') . '</b>: <img src="data:'.$picture_mime.';base64,'.base64_encode($picture).'" width="100%" height="100%"><br>';
            }
        }
        break;
