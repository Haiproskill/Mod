...
if (isset($_GET['lng'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        require('system/head.php');
        echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . _t('External Link', 'system') . '</h4></div>' .
            '<div class="list1">' .
            '<form action="go.php?url=' . rawurlencode($url) . '" method="post">' .
            '<h5>' . _t('ATTENTION!', 'system') . '</h5><p>' .
            _t('You are going to leave our site and go to an external link', 'system') . ': <span class="red">' . htmlspecialchars($url) . '</span></p><br />' .
            '<p>' . _t('Administration of our site is not responsible for the content of external sites', 'system') . '.<br />' .
            sprintf(_t('It is recommended not to specify your data, relating to %s (Login, Password), on third party sites', 'system'), '<span class="wgreen text--bold">' . $config->homeurl . '</span>') . '.</p>' .
            '<div class="button-container t10"><button  type="submit" name="submit" class="button"><span>' . _t('Go to Link', 'system') . '</span></button></div>' .
            '</form></div>' .
            '<div class="list1"><a href="' . $referer . '">' . _t('Back', 'system') . '</a></div></div>';
        require('system/end.php');
    }
} else if ($id) {
    /** @var PDO $db */
