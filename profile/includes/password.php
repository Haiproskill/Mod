...
switch ($mod) {
    ...
    default:
        // Форма смены пароля
        echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . _t('Change Password') . ':&#160;' . $user['name'] . '</h4></div>';
        echo '<form action="?act=password&amp;mod=change&amp;user=' . $user['id'] . '" method="post">';

        if ($user['id'] == $systemUser->id) {
            echo '<div class="list1"><div class="form-group"><input type="password" name="oldpass" required="required" /><label class="control-label" for="input">' . _t('Enter old password') . '</label><i class="bar"></i></div></div>';
        }

        echo '<div class="menu">' .
            '<div class="form-group"><input type="password" name="newpass" required="required" /><label class="control-label" for="input">' . _t('Enter new password') . '</label><i class="bar"></i></div>' .
            '<div class="form-group"><input type="password" name="newconf" required="required" /><label class="control-label" for="input">' . _t('Repeat password') . '</label><i class="bar"></i></div>' .
            '<div class="button-container"><button class="button" type="submit" name="submit"><span>' . _t('Save') . '</span></button></div>' .
            '</div></form>' .
            '<div class="list1"><a href="?user=' . $user['id'] . '">' . _t('Profile') . '</a></div></div>';
}
