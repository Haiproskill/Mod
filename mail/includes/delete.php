...
echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . _t('Deleting messages') . '</h4></div>';
...
if ($id) {
    ...
    if (isset($_POST['submit'])) { //Если кнопка "Подвердить" нажата
        ...
        if ($res['sys']) {
            ...
            echo '<div class="gmenu text-center">' . _t('Message deleted') . '</div>';
        ...
        echo '<div class="rmenu text-center">' . _t('You really want to remove the message?') . '</div>
        <div class="list1">
        <form action="index.php?act=delete&amp;id=' . $id . '" method="post">
        <div class="button-container"><button class="button" type="submit" name="submit"><span>' . _t('Delete') . '</span></button></div>
        </form></div>';
    }
} else {
    echo '<div class="rmenu text-center">' . _t('The message for removal isn\'t chosen') . '</div>';
}
