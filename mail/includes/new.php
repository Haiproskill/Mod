...
echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . _t('New Messages') . '</h4></div>';
...
if ($total == 1) {
    ...
    $res = $db->query("SELECT `user_id` FROM `cms_mail` WHERE `from_id`='" . $systemUser->id . "' AND `read`='0' AND `sys`!='1' AND `delete` != " . $systemUser->id)->fetch();
...
if ($total) {
    ...
    for ($i = 0; ($row = $query->fetch()) !== false; ++$i) {
        ...
        $count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE ((`user_id`='{$row['id']}' AND `from_id`='" . $systemUser->id . "') OR (`user_id`='" . $systemUser->id . "' AND `from_id`='{$row['id']}')) AND `delete`!='" . $systemUser->id . "' AND `spam`='0' AND `sys`!='1' ")->fetchColumn();
        $new_count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `cms_mail`.`user_id`='{$row['id']}' AND `cms_mail`.`from_id`='" . $systemUser->id . "' AND `read`='0' AND `delete`!='" . $systemUser->id . "' AND `spam`='0' AND `sys`!='1' ")->fetchColumn();
        $arg = [
