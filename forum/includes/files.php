...
if ($c || $s || $t) {
    ...
    $req = $db->query("SELECT `text`, `seo` FROM `forum` WHERE `id` = '$id'");
...
if ($do || isset($_GET['new'])) {
    ...
    if ($total) {
        // Заголовок раздела
        echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . $caption . (isset($_GET['new']) ? '&#160;|&#160;' . _t('New Files') : '') . '</h4></div>' . ($do ? '<div class="bmenu">' . $types[$do] . '</div>' : '');
        ...
        for ($i = 0; $res = $req->fetch(); ++$i) {
            ...
            // Выводим текст поста
            $req_nt = $db->query("SELECT `seo` FROM `forum` WHERE `id` = '" . $res['topic'] . "'")->fetch();
            ...
            $text = '<b><a href="' . $config['homeurl'] . '/forum/' . $res['topic'] . '/' . $req_nt['seo'] . '_p' . $page . '.html">' . $res['topicname'] . '</a></b><br />' . $text;

            if (mb_strlen($res['text']) > 500) {
                $text .= '<br /><a href="' . $config['homeurl'] . '/forum/post-' . $res['post'] . '.html">' . _t('Read more') . ' &gt;&gt;</a>';
            }

            // Формируем ссылку на файл
            $fls = @filesize('../files/forum/attach/' . $res['filelink']);
            $fls = round($fls / 1024, 0);
            $att_ext = strtolower(pathinfo('./files/forum/attach/' . $res['filelink'], PATHINFO_EXTENSION));
            $pic_ext = [
                'gif',
                'jpg',
                'jpeg',
    ...
    echo '<p>' . ($countnew > 0
            ...
            : _t('No new files')) . '</p>';
    echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . $caption . '</h4></div>';
    $link = [];
    $total = 0;
    for ($i = 1; $i < 10; $i++) {
...
echo '<p>' . (($do || isset($_GET['new']))
        ...
        : '') . '<a href="/forum/' . ($id ? '' . $id . '/' . $seo . '.html' : '') . '">' . _t('Forum') . '</a></p>';
