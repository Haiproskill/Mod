...
if ($act && ($key = array_search($act, $array)) !== false && file_exists('includes/' . $array[$key] . '.php')) {
    ...
    // Главное меню FAQ
    echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . _t('Information, FAQ') . '</h4></div>' .
        '<div class="list1"><a href="?act=forum">' . _t('Forum rules') . '</a></div>' .
        '<div class="list1"><a href="?act=tags">' . _t('bbCode Tags') . '</a></div>' .
        '<div class="list1"><a href="?act=smilies">' . _t('Smilies') . '</a></div></div>' .
        '<div class="mrt-code card shadow--2dp"><div class="phdr"><a href="' . $_SESSION['ref'] . '">' . _t('Back') . '</a></div></div>';
}
