...
$comment = isset($_REQUEST['comment']) ? abs(intval($_REQUEST['comment'])) : 0;
$reply   = isset($_REQUEST['reply']) ? abs(intval($_REQUEST['reply'])) : 0;

$toREP = false;
$toCMT = false;

$check = $db->query("SELECT COUNT(*) FROM `cms_users_guestbook` WHERE `id` = '" . $id . "' ")->fetchColumn();
if ($check) {
    ...
    if ($reply) {
        $sttrep = $db->query("SELECT `id`, `post_id` FROM `cms_users_guestbook_comments` WHERE `id`='" . $reply . "' AND `type`='reply' ");
        if ($sttrep->rowCount()) {
            $sttrepRES = $sttrep->fetch();
            $sttcmt = $db->query("SELECT `id`, `post_id` FROM `cms_users_guestbook_comments` WHERE `id`='" . $sttrepRES['post_id'] . "' AND `type`='cmt' ");
            if ($sttcmt->rowCount()) {
               $sttcmtRES = $sttcmt->fetch();
               $toREP = true;
               $toCMT = true;
            }
        }
    } elseif ($comment) {
        $sttcmt = $db->query("SELECT `id`, `post_id` FROM `cms_users_guestbook_comments` WHERE `id`='" . $comment . "' AND `type`='cmt' ");
        if ($sttcmt->rowCount()) {
            $sttcmtRES = $sttcmt->fetch();
            $toCMT = true;
        }
    ...
    $req = $db->query('SELECT `cms_users_guestbook`.*, `cms_users_guestbook`.`id` AS `stid`, `users`.`rights`, `users`.`sex`, `users`.`name`, `users`.`id`, `users`.`avatar_extension`
        ...
        WHERE `cms_users_guestbook`.`id` = "' . $id . '" LIMIT 1');
    ...
    while ($res = $req->fetch()) {
        ...
        echo '</div>';
            ...
            echo '<div class="status_comments list1"' . ($toCMT || $toREP ? ' style="display:block;"' : '') . '>' .
                                    ...
                                    '<div class="comment_content">' .
                                        '<div class="comment">' .
                                            '<div>' .
                                                ...
                                                ' ' . $post .
                                            '</div>' .
                                            '<div class="cm_time"><span class="status_other_data gray"><i class="material-icons">&#xE192;</i> '.(round((time()-$resComments['time'])/3600) < 2 ? '<span class="ajax-time" title="' . $tools->timestamp($resComments['time']) . '">':'') . $tools->thoigian($resComments['time']) . (round((time()-$resComments['time'])/3600) < 2 ? '</span>':'') . '</span>' .
                                            '</div>' .
