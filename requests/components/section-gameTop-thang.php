...
$req = $db->query("SELECT `id`, `name`, `sex`, `status`, `rights`, `balansUp` FROM `users` WHERE `preg` = 1 ORDER BY `balansUp` DESC LIMIT $start, $kmess");
if ($req->rowCount()) {
	$i = 0;
	while ($res = $req->fetch()) {
        ...
        $top[$i]['balans'] = $tools->balans($res['balansUp']);
