...
switch ($mod) {
    case 'del':
        ...
        $req = $db->query("SELECT * FROM `forum` WHERE `id` = '$id' AND (`type` = 'f' OR `type` = 'r' OR `type` = 'c')");

        if ($req->rowCount()) {
            $res = $req->fetch();
            echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4>' . ($res['type'] == 'r' ? _t('Delete section') : _t('Delete category')) . ': ' . $res['text'] . '</h4></div>';
            // Проверяем, есть ли подчиненная информация
            $total = $db->query("SELECT COUNT(*) FROM `forum` WHERE `refid` = '$id' AND (`type` = 'f' OR `type` = 'r' OR `type` = 'c' OR `type` = 't')")->fetchColumn();
            ...
            if ($total) {
                if ($res['type'] == 'f') {
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        echo '<form action="index.php?act=forum&amp;mod=del&amp;id=' . $id . '" method="POST">' .
                            '<div class="list1">' . _t('<h3>WARNING!</h3>There are subsections. Move them to another category.') . '<br /><br />' .
                            '<h3>' . _t('Select category') . '</h3><select name="category" size="1">';
                        $req_c = $db->query("SELECT * FROM `forum` WHERE `type` = 'f' AND `id` != '$id' ORDER BY `realid` ASC");

                        $i=0;
                        while ($res_c = $req_c->fetch()) {
                            echo '<option value="' . $res_c['id'] . '" ' . (!$i ? 'selected="selected"' : '') . '>' . $res_c['text'] . '</option>';
                            $i++;
                        }

                        echo '</select><br /><br /><span class="red">' . _t('All categories, topics, and files will be moved into selected category. Old category will be removed.') . '</span>' .
                            '<div class="button-container"><button type="submit" name="submit" class="button"><span>' . _t('Move') . '</span></button></div>' .
                            '</div>';
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        $check = $db->query("SELECT COUNT(*) FROM `forum` WHERE `id` = '$subcat' AND (`type` = 'r' OR `type` = 'c') ")->fetchColumn();
                        ...
                        if ($systemUser->rights != 9) {
                            echo $tools->displayError(_t('Access forbidden'), '', 'text-center');
                            echo '</div>';
                        ...
                        echo '<div class="rmenu text-center"><p>' . _t('Section with all contents are removed') . '</p></div>' .
                            '<div class="menu"><a href="index.php?act=forum&amp;mod=cat&amp;id=' . $res['refid'] . '">' . _t('Go to category') . '</a></div>';
                    } else {
                        echo '<form action="index.php?act=forum&amp;mod=del&amp;id=' . $id . '" method="POST"><div class="list1">' .
                            _t('<h3>WARNING!</h3>There are topics in the section. You must move them to another section.') . '<br /><br />' . '<h3>' . _t('Select section') . '</h3>';
                        $cat = isset($_GET['cat']) ? abs(intval($_GET['cat'])) : 0;
                        $ref = $cat ? $cat : $res['refid'];
                        $req_r = $db->query("SELECT * FROM `forum` WHERE `refid` = '$ref' AND `id` != '$id' AND (`type` = 'r' OR `type` = 'c') ORDER BY `realid` ASC");

                        $i=0;
                        while ($res_r = $req_r->fetch()) {
                            echo '<div class="form-radio radio"><label><input type="radio" name="subcat" value="' . $res_r['id'] . '" ' . (!$i ? 'checked="checked"' : '') . '/><i class="helper"></i>' . $res_r['text'] . '</label></div>';
                            $i++;
                        }

                        echo '<h3>' . _t('Other category') . '</h3><ul>';
                        ...
                        echo '</ul><br /><span class="red">' . _t('All the topics and files will be moved to selected section. Old section will be deleted.') . '</span><div class="button-container"><button type="submit" name="submit" class="button"><span>' . _t('Move') . '</span></button></div></div>';

                        if ($systemUser->rights == 9) {
                            // Для супервайзоров запрос на полное удаление
                            echo '<div class="menu"><a href="index.php?act=forum&amp;mod=cat">' . _t('Back') . '</a></div></div><div class="mrt-code card shadow--2dp"><div class="phdr"><h2>' .  _t('Complete removal') . '</h2></div>' .
                            '<div class="rmenu text-center">' . _t('WARNING! All the information will be deleted') . '</div>' .
                            '<div class="list1"><div class="button-container"><button type="submit" name="delete" class="button" style="margin:0;"><span>' . _t('Delete') . '</span></button></div></div>';
                        }
                ...
                if (isset($_POST['submit'])) {
                    $db->exec("DELETE FROM `forum` WHERE `id` = '$id'");
                    echo '<div class="gmenu text-center">' . ($res['type'] == 'r' || $res['type'] == 'c' ? _t('Section deleted') : _t('Category deleted')) . '</div>';
                } else {
                    echo '<div class="rmenu text-center">' . _t('Do you really want to delete?') . '</div>' .
                        '<div class="list1"><form action="index.php?act=forum&amp;mod=del&amp;id=' . $id . '" method="POST">' .
                        '<div class="button-container"><button type="submit" name="submit" class="button" style="margin:0;"><span>' . _t('Delete') . '</span></button></div>' .
                        '</form></div>';
                }
            }
            echo '<div class="menu"><a href="index.php?act=forum&amp;mod=cat">' . _t('Back') . '</a></div></div>';
        } else {
    ...
    case 'add':
        ...
        if ($id) {
            ...
            $req = $db->query("SELECT * FROM `forum` WHERE `id` = '$id' AND `type` != 't' AND `type` != 'm' ");
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!$error) {
                ...
                $req = $db->query("SELECT `realid` FROM `forum` WHERE " . ($id ? "`refid` = '$id' AND `type` = '$type' " : "`type` = 'f'") . " ORDER BY `realid` DESC LIMIT 1");
                ...
                echo $tools->displayError($error, '', 'text-center');
            }
        } else {
            // Форма ввода
            echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h2>' . ($id ? _t('Add Section') : _t('Add Category')) . '</h2></div>';
            ...
            echo '<form action="index.php?act=forum&amp;mod=add' . ($id ? '&amp;id=' . $id : '') . '" method="post">' .
                ...
                '<input type="text" name="name" maxlength="255" value="" placeholder="' . _t('Min. 2, Max. 255 characters') . '" />' .
                '<label class="control-label" for="input">' . _t('Title') . '</label><i class="bar"></i>' .
                '</div>' .
                '<div class="form-group"><textarea id="" rows="' . $systemUser->getConfig()->fieldHeight . '" name="desc" placeholder="' . _t('Min. 2, Max. 500 characters') . '"></textarea>' .
                '<label class="control-label" for="ctextarea">' . _t('Description') . ' (' . _t('Optional field') . ')</label><i class="bar"></i>' .
                '</div>';

            if ($id) {
                echo '<div class="form-radio radio"><label><input type="radio" name="allow" value="0" checked="checked"><i class="helper"></i>' . _t('Common access') . '</label></div>' .
                    '<div class="form-radio radio"><label><input type="radio" name="allow" value="4"><i class="helper"></i>' . _t('Only for reading') . '</label></div>' .
                    '<div class="form-radio radio"><label><input type="radio" name="allow" value="2"><i class="helper"></i>' . _t('Allow authors to edit the 1st post') . '</label></div>' .
                    '<div class="form-radio radio"><label><input type="radio" name="allow" value="1"><i class="helper"></i>' . _t('Assign the newly created authors as curators') . '</label></div>';
            }

            echo '<div class="button-container"><button class="button" type="submit" name="submit"><span class="flex-center">' . _t('Add') . '</span></button></div>' .
                '</div></form>' .
                '<div class="menu"><a href="index.php?act=forum&amp;mod=cat' . ($id ? '&amp;id=' . $id : '') . '">' . _t('Back') . '</a></div></div>';
        }
        break;
        ...
        if ($req->rowCount()) {
            ...
            if ($res['type'] != 't' || $res['type'] != 'm') {
                if (isset($_POST['submit'])) {
                    ...
                    if (($res['type'] == 'r' || $res['type'] == 'c') && !$category) {
                        $error[] = _t('Category is not chosen');
                    } else if (($res['type'] == 'r' || $res['type'] == 'c') && !$db->query("SELECT COUNT(*) FROM `forum` WHERE `id` = '$category' ")->fetchColumn()) {
                        $error[] = _t('Category is not chosen');
                    }
                    ...
                    if (!$error) {
                        ...
                        if (($res['type'] == 'r' || $res['type'] == 'c') && $category != $res['refid']) {
                            if ($res['type'] == 'r') {
                                $type_r = 'r';
                            } else {
                                $type_r = 'c';
                            ...
                            $req_s = $db->query("SELECT `realid` FROM `forum` WHERE `refid` = '$category' AND `type` = '$type_r' ORDER BY `realid` DESC LIMIT 1");
                            $res_s = $req_s->fetch();
                            $sort = $res_s['realid'] + 1;
                            // Меняем категорию
                        ...
                        header('Location: index.php?act=forum&mod=cat' . ($res['type'] == 'r' || $res['type'] == 'c' ? '&id=' . $res['refid'] : ''));
                    } else {
                        // Выводим сообщение об ошибках
                    ...
                    // Форма ввода
                    echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h2>' . ($res['type'] == 'r' ? _t('Edit Section') : _t('Edit Category')) . '</h2></div>' .
                        '<form action="index.php?act=forum&amp;mod=edit&amp;id=' . $id . '" method="post">' .
                        '<div class="list1">' .
                        '<div class="form-group">' .
                        '<input type="text" name="name" maxlength="255" value="' . $res['text'] . '" placeholder="' . _t('Min. 2, Max. 255 characters') . '" />' .
                        '<label class="control-label" for="input">' . _t('Title') . '</label><i class="bar"></i>' .
                        '</div>' .
                        '<div class="form-group"><textarea id="" rows="' . $systemUser->getConfig()->fieldHeight . '" name="desc" placeholder="' . _t('Min. 2, Max. 500 characters') . '">' . str_replace('<br>', "\r\n", $res['soft']) . '</textarea>' .
                        '<label class="control-label" for="ctextarea">' . _t('Description') . ' (' . _t('Optional field') . ')</label><i class="bar"></i>' .
                        '</div>';

                    if ($res['type'] == 'r' || $res['type'] == 'c') {
                        $allow = !empty($res['edit']) ? intval($res['edit']) : 0;
                        echo '<div class="form-radio radio"><label><input type="radio" name="allow" value="0" ' . (!$allow ? 'checked="checked"' : '') . '><i class="helper"></i>' . _t('Common access') . '</label></div>' .
                            '<div class="form-radio radio"><label><input type="radio" name="allow" value="4" ' . ($allow == 4 ? 'checked="checked"' : '') . '><i class="helper"></i>' . _t('Only for reading') . '</label></div>' .
                            '<div class="form-radio radio"><label><input type="radio" name="allow" value="2" ' . ($allow == 2 ? 'checked="checked"' : '') . '><i class="helper"></i>' . _t('Allow authors to edit the 1st post') . '</label></div>' .
                            '<div class="form-radio radio"><label><input type="radio" name="allow" value="1" ' . ($allow == 1 ? 'checked="checked"' : '') . '><i class="helper"></i>' . _t('Assign the newly created authors as curators') . '</label></div>';

                        echo '<br /><p><h3>' . _t('Category') . '</h3><select name="category" size="1">';
                        if ($res['type'] == 'r') {
                            $req_c = $db->query("SELECT * FROM `forum` WHERE `type` = 'f' ORDER BY `realid` ASC");
                        } else if ($res['type'] == 'c'){
                            $res_cc = $db->query("SELECT * FROM `forum` WHERE `id` = '$res[refid]' ")->fetch();
                            $req_c = $db->query("SELECT * FROM `forum` WHERE `refid` = '$res_cc[refid]' ORDER BY `realid` ASC");
                        }
                        while ($res_c = $req_c->fetch()) {
                    ...
                    echo '<div class="button-container"><button class="button" type="submit" name="submit"><span class="flex-center">' . _t('Save') . '</span></button></div>' .
                        '</div></form>' .
                        '<div class="menu"><a href="index.php?act=forum&amp;mod=cat' . ($res['type'] == 'r' ? '&amp;id=' . $res['refid'] : '') . '">' . _t('Back') . '</a></div></div>';
                }
            } else {
                header('Location: index.php?act=forum&mod=cat');
            }
    ...
    case 'up':
        ...
        if ($id) {
            ...
            if ($req->rowCount()) {
                $res1 = $req->fetch();
                $sort = $res1['realid'];
                $req = $db->query("SELECT * FROM `forum` WHERE `type` = '" . $res1['type'] . "' AND `realid` < '$sort' ORDER BY `realid` DESC LIMIT 1");
        ...
        header('Location: index.php?act=forum&mod=cat' . ($res1['type'] != 'f' ? '&id=' . $res1['refid'] : ''));
        break;
    ...
    case 'down':
        ...
        if ($id) {
            ...
            if ($req->rowCount()) {
                $res1 = $req->fetch();
                $sort = $res1['realid'];
                $req = $db->query("SELECT * FROM `forum` WHERE `type` = '" . $res1['type'] . "' AND `realid` > '$sort' ORDER BY `realid` ASC LIMIT 1");
    ...
    case 'cat':
        ...
        echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><a href="index.php?act=forum"><b>' . _t('Forum Management') . '</b></a>&#160;|&#160;' . _t('Forum structure') . '</div></div>';

        if ($id) {
            // Управление разделами
            $req = $db->query("SELECT * FROM `forum` WHERE `id` = '$id' AND `type` != 't' AND `type` != 'm' ");
            $res = $req->fetch();
            echo '<div class="mrt-code card shadow--2dp"><div class="bmenu"><a href="index.php?act=forum&amp;mod=cat' . ($res['refid'] ? '&amp;id=' . $res['refid'] : '') . '"><b>' . $res['text'] . '</b></a> | ' . _t('List of sections') . '</div>';
            $req = $db->query("SELECT * FROM `forum` WHERE `refid` = '$id' AND `type` != 't' AND `type` != 'm' ORDER BY `realid` ASC");
            ...
            if ($req->rowCount()) {
                ...
                while ($res = $req->fetch()) {
                    ...
                    echo '<a href="index.php?act=forum&amp;mod=cat&amp;id=' . $res['id'] . '"><b>' . $res['text'] . '</b></a> ' .
                        '(' . $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'c' AND `refid` = '" . $res['id'] . "'")->fetchColumn() . ')' .
                ...
                echo '<div class="list1"><p>' . _t('The list is empty') . '</p></div>';
            }
        } else {
            // Управление категориями
            echo '<div class="mrt-code card shadow--2dp"><div class="bmenu">' . _t('List of categories') . '</div>';
        ...
        echo '<div class="menu">' .
            ...
            '<div class="button-container"><button class="button" type="submit" style="margin:0;"><span class="flex-center">' . _t('Add') . '</span></button></div>' .
            '</form></div></div>';
        break;

    case 'htopics':
        // Управление скрытыми темами форума
        echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4><a href="index.php?act=forum"><b>' . _t('Forum Management') . '</b></a>&#160;|&#160;' . _t('Hidden topics') . '</h4></div>';
        $sort = '';
        $link = '';
        ...
        if (isset($_POST['deltopic'])) {
            ...
            $req = $db->query("SELECT `forum`.*, `forum`.`id` AS `fid`, `forum`.`seo` AS `fseo`, `forum`.`user_id` AS `id`, `forum`.`from` AS `name`, `forum`.`soft` AS `browser`, `users`.`rights`, `users`.`lastdate`, `users`.`sex`, `users`.`status`, `users`.`datereg`
            ...
            if ($req->rowCount()) {
                ...
                while ($res = $req->fetch()) {
                    ...
                    $text = '<a href="../forum/' . $res['fid'] . '/' . $res['fseo'] . '.html"><b>' . $res['text'] . '</b></a>';
                    $text .= '<br><small><a href="../forum/' . $cat['id'] . '/' . $cat['seo'] . '.html">' . $cat['text'] . '</a> / <a href="../forum/' . $subcat['id'] . '/' . $subcat['seo'] . '.html">' . $subcat['text'] . '</a></small>';
                ...
                echo '<div class="gmenu text-center"><p>' . _t('The list is empty') . '</p></div>';
            }
            echo '</div>';
    ...
    case 'hposts':
        ...
        echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><h4><a href="index.php?act=forum"><b>' . _t('Forum Management') . '</b></a>&#160;|&#160;' . _t('Hidden posts') . '</h4></div>';
        $sort = '';
        $link = '';
        ...
        if (isset($_POST['delpost'])) {
            ...
            if ($req->rowCount()) {
                ...
                while ($res = $req->fetch()) {
                    ...
                    $theme = $db->query("SELECT `id`, `text`, `seo` FROM `forum` WHERE `id` = '" . $res['refid'] . "'")->fetch();
                    $text = '<b>' . $theme['text'] . '</b> <a href="/forum/' . $theme['id'] . '/' . $theme['seo'] . '_p' . $page . '.html">&gt;&gt;</a><br>' . $text;
                    $subtext = '<span class="gray">' . _t('Filter') . ':</span> ';
    ...
    default:
        ...
        $total_sub = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'r' or `type` = 'c' ")->fetchColumn();
        ...
        echo '<div class="mrt-code card shadow--2dp"><div class="phdr"><a href="index.php"><b>' . _t('Admin Panel') . '</b></a>&#160;|&#160;' . _t('Forum Management') . '</div>' .
            '<div class="list1"><p><h3>' . $tools->image('rate.gif') . _t('Statistic') . '</h3><ul>' .
            ...
            '</ul></p></div></div>' .
            '<div class="mrt-code card shadow--2dp"><div class="phdr"><h3><img src="../images/settings.png" width="16" height="16" class="left" />&#160;' . _t('Settings') . '</h3></div>' .
            '<div class="list1"><a href="index.php?act=forum&amp;mod=cat"><b>' . _t('Forum structure') . '</b></a></div>' .
            '<div class="list1"><a href="index.php?act=forum&amp;mod=hposts">' . _t('Hidden posts') . '</a> (' . $total_msg_del . ')</div>' .
            '<div class="list1"><a href="index.php?act=forum&amp;mod=htopics">' . _t('Hidden topics') . '</a> (' . $total_thm_del . ')</div>' .
            '</div>';
}

echo '<div class="mrt-code card shadow--2dp"><div class="card__actions"><a href="index.php">' . _t('Admin Panel') . '</a></div></div>';
